...
def nssfbdec(x, f1, f2, mup=None, gpu_mode=False, index_dict=None):
    if mup is None:
        y1 = efilter2(x, f1, gpu_mode=gpu_mode)
        y2 = efilter2(x, f2, gpu_mode=gpu_mode)
        return y1, y2
    ...
    if np.asarray(mup).shape == (2, 2):
        y1 = zconv2(x, f1, mup, gpu_mode=gpu_mode)
        y2 = zconv2(x, f2, mup, gpu_mode=gpu_mode)
