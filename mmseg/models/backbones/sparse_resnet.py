...
from ..utils.res_layer import build_sparse_conv_layer

from ..builder import BACKBONES
from ..utils import ResLayer, SpResLayer

from mmseg.utils import get_root_logger
from mmcv.runner import load_checkpoint


def to_sparse(x, th=0.7):
...
class Bottleneck(BaseModule):
    ...
    def __init__(self,
        ...
        # self.sp_block_1 = spconv.SparseSequential(self.conv1, norm1, nn.ReLU(inplace=True))
        self.sp_block_1 = spconv.SparseSequential(conv1, norm1, nn.ReLU(inplace=True))
        self.sp_block_2 = spconv.SparseSequential(conv2, norm2, nn.ReLU(inplace=True))
        self.sp_block_3 = spconv.SparseSequential(conv3, norm3)
        self.sp_relu = spconv.SparseSequential(nn.ReLU(inplace=True))
    ...
    def forward(self, x):
        ...
        def _inner_forward(x):
            ...
            out = self.sp_block_1(x)

            if self.with_plugins:
                out = self.forward_plugin(out, self.after_conv1_plugin_names)

            out = self.sp_block_2(out)

            if self.with_plugins:
                out = self.forward_plugin(out, self.after_conv2_plugin_names)

            out = self.sp_block_3(out)
            ...
            sparse_features = out.features + identity.features

            out = out.replace_feature(sparse_features)
        ...
        out = self.sp_relu(out)

        return out


@BACKBONES.register_module()
class SpResNet(BaseModule):
    """ResNet backbone.
    ...
    def __init__(self,
                 ...
                 pretrained=None,
                 init_cfg=None):
        super(SpResNet, self).__init__(init_cfg)
        ...
        for i, num_blocks in enumerate(self.stage_blocks):
            ...
            if plugins is not None:
                raise ValueError("Plugins is not supported in Sparse Convolution")
            else:
                stage_plugins = None
            ...
            res_layer = self.make_sp_res_layer(
                block=self.block,
    ...
    def make_sp_res_layer(self, **kwargs):

        return SpResLayer(**kwargs)
    ...
    def train(self, mode=True):
        ...
        freezed."""
        super(SpResNet, self).train(mode)
        self._freeze_stages()
