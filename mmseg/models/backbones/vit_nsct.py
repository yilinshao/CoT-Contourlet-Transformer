...
class VIT_NSCT_01(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.ms_nsct:
            x2_1 = F.interpolate(x2_1, x2_2.shape[-1] // 2, mode='bilinear', align_corners=True)
            low_freq = F.interpolate(low_freq, x2_2.shape[-1] // 4, mode='bilinear', align_corners=True)

        nsct_features = [self.resnet_backbone_1(x2_1)[0],
                         self.resnet_backbone_2(x2_2)[0]]

        tf_feature_c18, tf_feature_c24, nsct_1, nsct_2 = self.vit_backbone(x[:, 0: 3],
                                                               nsct_features=nsct_features,
                                                               low_freq=low_freq)
